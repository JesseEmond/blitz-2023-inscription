...
axs[1,1].set_title('score per local evaporation rate')
...
axs[1,1].plot('local_evaporation_rate', 'score', 'o', data=pts)

axs[2,1].set_title('score per min pheromones')
min_pheromones_pts = sorted(points, key=lambda p: p.hyperparams.min_pheromones)
pts = {'min_pheromones': [p.hyperparams.min_pheromones for p in min_pheromones_pts],
       'score': [p.score for p in min_pheromones_pts]}
axs[2,1].plot('min_pheromones', 'score', 'o', data=pts)

axs[2,2].set_title('score per max pheromones')
max_pheromones_pts = sorted(points, key=lambda p: p.hyperparams.max_pheromones)
pts = {'max_pheromones': [p.hyperparams.max_pheromones for p in max_pheromones_pts],
       'score': [p.score for p in max_pheromones_pts]}
axs[2,2].plot('max_pheromones', 'score', 'o', data=pts)

axs[1,2].set_title('score per pheromone init ratio')
pheromones_init_ratio_pts = sorted(points, key=lambda p: p.hyperparams.pheromones_init_ratio)
pts = {'pheromones_init_ratio': [p.hyperparams.pheromones_init_ratio for p in pheromones_init_ratio_pts],
       'score': [p.score for p in pheromones_init_ratio_pts]}
axs[1,2].plot('pheromones_init_ratio', 'score', 'o', data=pts)
